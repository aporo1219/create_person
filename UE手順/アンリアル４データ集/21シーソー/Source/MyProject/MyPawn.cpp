...
AMyPawn::AMyPawn()
{
	...
	mSpringArm->bDoCollisionTest = false;//カメラの衝突不可
	...
	//mStimuliSource->RegisterForSense(TSubclassOf<UAISense_Sight>());
	//mStimuliSource->RegisterWithPerceptionSystem();
	//mStimuliSource->bAutoRegister = true;
...
void AMyPawn::Tick(float DeltaTime)
{
	...
	{
		//移動
		mMove->AddInputVector(mMoveVec, true);
	}
	...
	//追加内容：AIによる追尾
	//UAIPerceptionComponentは、RootComponentのトランスフォームの位置に取るのでSetRelativeLocationをする
	FVector wpos;
	...
	//物理的なZ方向移動速度を確認
	//FVector vec = mCapsule->GetPhysicsLinearVelocity();
	//UKismetSystemLibrary::PrintString(this, FString::Printf(TEXT("t=%f"),vec.Z), true, true, FColor::Cyan, 2.f);
	//今の移動力を見る
	//UKismetSystemLibrary::PrintString(this, FString::Printf(TEXT("x=%f,y=%f"), mMove->Velocity.X, mMove->Velocity.Y), true, true, FColor::Cyan, 2.f);
...
void AMyPawn::Jump()
{
	...
	if (mCapsule->IsSimulatingPhysics())
	{
