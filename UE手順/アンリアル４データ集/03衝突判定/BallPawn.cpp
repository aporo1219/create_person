...
ABallPawn::ABallPawn()
{
	...
	mCubeMesh->BodyInstance.SetCollisionProfileName("Projectile");//コリジョンセット
	mCubeMesh->SetNotifyRigidBodyCollision(true);//コリジョンのHitイベント発行有効（Simulation Generates Hit Events）
...
{
	Super::BeginPlay();
	mMoveForce = 300000 / mCubeMesh->GetMass();
	mJumpCount = 0;
}
